...
def main():
    ...
    initial_positions = [0, 0, 0, 0.08, 0, 0, 0, 0, -0.08, 0]
...
def touchpiano():
    ...
    angle2 = 0.2
    angle3 = 0.5
    initial_positions = [0, 0, 0, 0.08, angle1, angle1, 0, angle1, -0.08, angle1]
    hand.set_all_active_joint_angles(initial_positions)
    print("初始位置：手指伸直")
    time.sleep(1)
    touch_positions = [0, 0, 0, 0.08, angle2, angle2, 0, angle2, -0.08, angle2]
    # 按下时其他手指角度为touch_positions,弯曲手指角度为angle3,抬起时角度恢复为initial_positions
    piano_sequence = [
        # 1155665-
        # 4s
        ('pinky', 0.5),   # 小指按下
    ...
    for finger_name, press_depth in piano_sequence:
        if finger_name == '-':
            ...
            continue
        joint_idx = finger_joints[finger_name]
        current_positions = touch_positions.copy()
        current_positions[joint_idx] = angle3
        hand.set_all_active_joint_angles(current_positions)
        print(f"按下 {finger_name} 手指")
        time.sleep(0.3)
        hand.set_all_active_joint_angles(initial_positions)
